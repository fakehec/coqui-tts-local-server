...
- `setup.sh` was tracked in git with mode `100644` (no exec bit), so a
  fresh `./setup.sh` after `git clone` failed with `bash: ./setup.sh:
  Permission denied`. Marked executable in the index (`100755`),
  matching every other repo in the Uttera stack.
...
- **Default port migrated from `5100` → `9004`.** Formalising the
  ...
  **Why not keep `5100`:** although `5100` itself had no mainstream
  collisions, the asymmetric pairing (TTS=5100 + STT=9005, because
  STT had to move off 5000 due to macOS AirPlay / Docker Registry v2
  collisions) would be surprising. Moving both families to the
  9000-9099 range keeps the pair adjacent and in an IANA "User
  Ports" range with no canonical assignment.
...
- `uttera-tts-vllm` v1.3.0 adopts the same `9004` port.
- `uttera-stt-hotcold` v2.3.0 and `uttera-stt-vllm` v1.3.0 adopt
  `9005` on the STT side.

## [2.2.1] - 2026-04-18

### Fixed

- `/health` `model` field now reports the actual model the active
  backend is running, not the global `TTS_MODEL` env var. v2.2.0
  showed `tts_models/multilingual/multi-dataset/xtts_v2` (the Coqui
  default) even when `TTS_BACKEND=voxcpm` was active and the
  backend had loaded `openbmb/VoxCPM2`. Cosmetic only — no
  behavioural change, just clearer observability.

### Added

- `TTSBackend.model_id()` — new method on the base class that
  backends override to expose the concrete model path / HF repo
  they loaded in `load()`. Plugin authors should add this to any
  third-party backends; the default returns `self.name` as a safe
  fallback.
  - Coqui backend returns `self._model_name`.
  - VoxCPM backend returns `self._model_id`.

### Changed

- **`SERVER_VERSION` bumped to `2.2.1`.**
...
- **`/v1/models` `owned_by`** now `"uttera"` (was the stale
  `"stark-fleet"` left over from the pre-rebrand release).
