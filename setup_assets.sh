...
echo "🦾 J.A.R.V.I.S. - Provisioning Infrastructure Assets (Local Mode)..."
...
for voice in "${voices[@]}"; do
    ...
    if [ ! -f "$TARGET_FILE" ]; then
        ...
        # -4: force IPv4. cdn.openai.com is on Azure CDN which blocks IPv6 from
        # Hurricane Electric tunnel broker ASNs (2001:470::/32), causing the TLS
        # handshake to hang indefinitely on systems using HE IPv6 tunnels.
        curl -L -s -4 -o "$TARGET_FILE" "$VOICE_BASE_URL/$voice.wav"
